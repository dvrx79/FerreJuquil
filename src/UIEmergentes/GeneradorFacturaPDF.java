...
public class GeneradorFacturaPDF {
    ...
    public void generarFactura(DatosFactura datos) {
        ...
        try {
            ...
            InputStream pdfInputStream = GeneradorFacturaPDF.class.getResourceAsStream("Referencia_FACTURA.pdf");
            ...
            try (PDDocument document = Loader.loadPDF(pdfBytes)) {
                ...
                try (PDPageContentStream cs = new PDPageContentStream(document, page, PDPageContentStream.AppendMode.APPEND, true, true)) {
                    ...
                    float x = 460, y = 705, lh = 25;
                    cs.setNonStrokingColor(new PDColor(new float[]{1, 0, 0}, PDDeviceRGB.INSTANCE));
                    cs.setFont(fontBold, 11);
                    escribir(cs, x + 13, y, datos.folio); y -= lh;

                    cs.setNonStrokingColor(Color.BLACK);
                    cs.setFont(fontNormal, 9);
                    escribir(cs, x - 128, y, datos.uuid); y -= lh;
                    escribir(cs, x - 40, y, datos.numeroCertificado); y -= lh;

                    cs.setFont(fontNormal, 10);
                    escribir(cs, x - 35, y - 2, datos.fecha);

                    // === Cliente ===
                    x = 142; y = 602; lh = 13;
                    cs.setFont(fontBold, 10);
                    escribir(cs, x, y, datos.nombreCliente); y -= lh;
                    escribir(cs, x, y, datos.rfcCliente); y -= lh;
                    escribir(cs, x, y, datos.direccionCliente); y -= lh;
                    escribir(cs, x, y, datos.ciudadEstadoCP); y -= lh;
                    escribir(cs, x, y - 2, datos.telefono);

                    // === Tabla productos ===
                    float startX = 80, startY = 497, separacionX = 65, fontSize = 8;
                    ...
                    cs.setFont(fontBold, 10);
                    escribir(cs, 240, 146, datos.usoCFDI);
                    escribir(cs, 194, 133, datos.tipoPersona);
                ...
                String baseFileName = datos.folio + nombreClienteLimpio + "_" + fechaHoy;
